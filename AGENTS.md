...
- The agent is responsible for maintaining the product version in `product/dee-ddp71-atmos-wrapper.py`.
- Increase the product version when development reaches an appropriate version boundary. Use semantic intent: patch for compatible fixes, minor for compatible features, and major for incompatible behavior or interface changes. Documentation-only edits normally do not require a product-version bump.
- The product version is a development-stream identifier. A packaged release identifier may be maintained separately and does not have to match it.
- Do not change historical validation metadata merely to make it match a newer product or release version. Update validation metadata only when the corresponding validation is actually rerun for that product state.
...
- Record the packaged release identifier in both release README files. Do not change the embedded product version solely to match a release identifier.
...
- For release identifier `<version>`, use package name `dee-ddp71-atmos-wrapper-<version>` consistently:
...
2. Decide whether the product version should be increased for the accumulated development changes; update it when appropriate.
...
6. Maintain the release README files independently and record the release identifier in them. Update the bilingual `RELEASE_NOTES.md` for the packaged snapshot.
