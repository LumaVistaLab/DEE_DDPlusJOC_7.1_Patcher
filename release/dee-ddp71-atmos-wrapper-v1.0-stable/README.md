...
> Local release: `v1.0-stable`. This release supports one exact verified build of `dee_audio_filter_ddp_atmos.dll`. Its original SHA-256 must be `3d66bcec36031fd48e6565d15f05fea656642377ca4f8c98cdce1cce8b7e95d2`.
...
Generated job XML, logs, intermediate streams, and the run manifest are under `work/runs/<run-id>/`. Verified component backups are stored under `backups/<installation-id>/`.
...
Before packaging, the main wrapper and independent Surround EX tool passed Python compilation checks, and all 14 source-tree automated tests passed. Real encoding was also validated on 2026-09-06 with both absolute and relative Windows paths containing valid Unicode and punctuation. Development tests and detailed validation records remain in the source repository and are not included in this runtime package.

The release identifier is recorded independently from the product's embedded development-stream version. See [RELEASE_NOTES.md](RELEASE_NOTES.md) for the packaged release history.
