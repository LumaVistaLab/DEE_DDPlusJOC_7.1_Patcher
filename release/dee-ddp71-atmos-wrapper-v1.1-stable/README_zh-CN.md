...
> 本产品版本只支持经过验证的 `dee_audio_filter_ddp_atmos.dll` 精确构建。原始文件 SHA-256 必须为 `3d66bcec36031fd48e6565d15f05fea656642377ca4f8c98cdce1cce8b7e95d2`。
...
作业 XML、日志、中间码流及运行清单位于 `work/runs/<运行标识>/`。运行时生成的 `backups/` 和 `work/` 目录不属于发布包。
...
打包产品版本 `v1.1-stable` 前，主包装器、Surround EX 独立工具和 DEE 暂存区清理工具均通过 Python 编译检查，27 项包装器自动化测试于 2026-09-07 全部通过。2026-09-06 绝对路径和相对路径实机编码的历史源码树记录保持不变，不作为本产品版本重新执行实机编码的声明。

解包后请使用 `SHA256SUMS.txt` 校验全部文件。产品版本与产品内置的内部版本分别维护。本次打包快照的变更和限制见 [RELEASE_NOTES.md](RELEASE_NOTES.md)。
